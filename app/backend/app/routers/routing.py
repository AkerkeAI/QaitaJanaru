...
from app.services.task_service import record_map_visit, record_route_open
...
async def get_navigation(request: NavigationRequest, db: Session = Depends(get_db)):
    """
    ...
    **Errors:**
    ...
    try:
        ...
        if not (
            -90 <= request.destination_lat <= 90
            and -180 <= request.destination_lon <= 180
        ):
            raise HTTPException(
                status_code=400, detail="Invalid destination coordinates"
            )
        ...
        raise HTTPException(status_code=503, detail="Unable to calculate route")
